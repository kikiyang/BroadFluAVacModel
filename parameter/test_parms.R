...
# sim.summary <- function(parms2fit){
#   const_parms <- c(R0_1=1.44,R0_2=1.60,sigma1=1/3.12,sigma2=1/2.28,mu=1/75, #from Yang et al., 2020
#                    theta1=0.4,theta2=0.17,gamma1=365/2.64,gamma2=365/3.03, #from Yang et al., 2020
#                    a1=0.1,a2=0.1,sigmaV=0,
#                    # sigmaT=SIGMAT[idx],
#                    sigmaT=365/(30*2), #from Ferguson et al., 2003
#                    nu1=0,nu2=0,rho=0,rho1=0,rho2=0,eta=0,eta2=0,
...
#   return (sim.stats)
# }
# sse.sir.obj.new <- function(parms2fit_log){
#   parms2fit <- exp(parms2fit_log)
#   sim.stats <- sim.summary(parms2fit)
#   
#   inci.total1.scale <- sim.stats$inci.total1
#   inci.total2.scale <- sim.stats$inci.total2
#   
#   epi.size1.sim <- log(10^(-100)+inci.total1.scale)
#   epi.size2.sim <- log(10^(-100)+inci.total2.scale)
#   
#   epi.size1.data <- log(stats.season[['inci.total1']])
#   epi.size2.data <- log(stats.season[['inci.total2']])
#   
#   mean.h1 <- mean(inci.total1.scale)
#   mean.h3 <- mean(inci.total2.scale)
#   cor.h1h3 <- ifelse(is.na(cor(epi.size1.sim,epi.size2.sim)),0,cor(epi.size1.sim,epi.size2.sim))
#   acf.h1 <- ifelse(is.na(acf(epi.size1.sim, plot=FALSE)[[1]][2,,1]),0,acf(epi.size1.sim, plot=FALSE)[[1]][2,,1])
#   acf.h3 <- ifelse(is.na(acf(epi.size2.sim, plot=FALSE)[[1]][2,,1]),0,acf(epi.size2.sim, plot=FALSE)[[1]][2,,1])
#   sd.h1 <- sd(epi.size1.sim)
#   sd.h3 <- sd(epi.size2.sim)
#   
#   optim_metrics <- data.frame(mean.h1,mean.h3,cor.h1h3,acf.h1,acf.h3,sd.h1,sd.h3)
#   # return(optim_metrics)
#   dis.mean.h1 <- abs(mean(epi.size1.sim) - mean(epi.size1.data))/abs(mean(epi.size1.data))
#   dis.mean.h3 <- abs(mean(epi.size2.sim) - mean(epi.size2.data))/abs(mean(epi.size2.data))
#   
#   dis.cor.h1h3 <- abs(ifelse(is.na(cor(epi.size1.sim,epi.size2.sim)),0,cor(epi.size1.sim,epi.size2.sim))-
#                         cor(epi.size1.data,epi.size2.data))/abs(cor(epi.size1.data,epi.size2.data))
#   
#   dis.acf.h1 <- abs(ifelse(is.na(acf(epi.size1.sim, plot=FALSE)[[1]][2,,1]),0,acf(epi.size1.sim, plot=FALSE)[[1]][2,,1])-
#                       acf(epi.size1.data, plot=FALSE)[[1]][2,,1])/abs(acf(epi.size1.data, plot=FALSE)[[1]][2,,1])
#   
#   dis.acf.h3 <- abs(ifelse(is.na(acf(epi.size2.sim, plot=FALSE)[[1]][2,,1]),0,acf(epi.size2.sim, plot=FALSE)[[1]][2,,1])-
#                       acf(epi.size2.data, plot=FALSE)[[1]][2,,1])/abs(acf(epi.size2.data, plot=FALSE)[[1]][2,,1])
#   
#   dis.sd.h1 <- abs(sd(epi.size1.sim) - sd(epi.size1.data))/abs(sd(epi.size1.data))
#   dis.sd.h3 <- abs(sd(epi.size2.sim) - sd(epi.size2.data))/abs(sd(epi.size2.data))
#   
#   eucldist <- sum(dis.mean.h1, dis.mean.h3, dis.sd.h1, dis.sd.h3,
#                   dis.cor.h1h3, dis.acf.h1, dis.acf.h3)
#   
#   optimdists <- data.frame(dis.mean.h1, dis.mean.h3, dis.sd.h1, dis.sd.h3,
#                            dis.cor.h1h3, dis.acf.h1, dis.acf.h3, eucldist)
#   #print(eucldist)
#   return(optimdists)
# }
# optimdists <- data.frame()
# for (idx in 1:length(theta1)){
#   #  idx2<-5
#   #  optimdists_tmp <- sim.summary(c(theta1[idx],theta2[idx2]))
#   for (idx2 in 1:length(theta2)){
#     optimdists_tmp <- sse.sir.obj.new(log(c(theta1[idx],theta2[idx2])))
#     optimdists_tmp['theta1'] <- theta1[idx]
#     optimdists_tmp['theta2'] <- theta2[idx2]
#     optimdists <- bind_rows(optimdists,optimdists_tmp)
#   }
# }
# 
# ggplot(optimdists %>% filter(theta1==0.25 & time>2250),aes(time,inci1))+geom_line()
# ggplot(optimdists %>% filter(theta1==0.26 & time>2250),aes(time,inci1))+geom_line()
# 
# ggplot(optimdists %>% filter(theta1==0.25 & time>2250),aes(time,inci2))+geom_line()
# ggplot(optimdists %>% filter(theta1==0.26 & time>2250),aes(time,inci2))+geom_line()
# 
# 
# ggplot(optimdists,aes(theta1,mean.h3))+geom_point()
# ggplot(optimdists,aes(theta1,cor.h1h3))+geom_point()
# ggplot(optimdists,aes(theta1,acf.h1))+geom_point()
# ggplot(optimdists,aes(theta1,acf.h3))+geom_point()
# ggplot(optimdists,aes(theta1,sd.h1))+geom_point()
# ggplot(optimdists,aes(theta1,sd.h3))+geom_point()
# 
# ggplot(data=optimdists %>% filter(theta2<0.8)) +
#   geom_raster(aes(theta1,theta2,fill=eucldist))
#   scale_fill_viridis_c()
