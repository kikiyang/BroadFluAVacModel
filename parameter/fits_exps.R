...
sirmod <- function(t,y,parms){
  with(as.list(c(y, parms)), {
    ...
    dR1 <- sigma2*R12 +sigmaT*RT1 +sigmaV*RV1 -
      beta2*(1-theta1)*R1*(I2+I12+IV2) -(mu+sigma1+rho)*R1 #infection-induced immunized against strain1
    ...
    dR2 <- sigma1*R12 +sigmaT*RT2 +sigmaV*RV2 - 
      beta1*(1-theta2)*R2*(I1+I21+IV1) -(mu+sigma2+rho)*R2 #infection-induced immunized against strain2
    ...
    dRV1 <- sigmaT*RVT1 + rho*R1-(sigma1+sigmaV+mu)*RV1
    ...
    dRV2 <- sigmaT*RVT2 +rho*R2 -(sigma2+sigmaV+mu)*RV2 #vaccinated individuals recovered from strain2 infection 
...
bestfitsparms[3,'theta1'] <- 0.55
bestfitsparms[3,'theta2'] <- 0.35
bestfitsparms[3,'a'] <- 0.04
for (idx in 1:nrow(bestfitsparms)){
  # initial parameter (w/ vaccine)
  parms0 <- c(R0_1=1.44,R0_2=1.60,a1=bestfitsparms[idx,'a'],
              a2=bestfitsparms[idx,'a'],sigma1=1/3.12,sigma2=1/2.28,sigmaV=0,
              gamma1=365/2.64,gamma2=365/3.03,mu=1/75,nu1=0,nu2=0,rho=0,
              eta=0,eta2=0,tau=0,tau2=0,sigmaT=365/(30*2),
  ...
  parms1['sigmaV'] <- 2/(3.12 + 2.28)
  
  # H1 vaccine
  parms2 <- parms1
  # parms2['tau2'] <- parms1['theta2']
  parms2['tau'] <- 0
  parms2['sigmaV'] <- 1/3.12
  
  # H3 vaccine
  parms3 <- parms1
  # parms3['tau'] <- parms1['theta1']
  parms3['tau2'] <- 0
  parms3['sigmaV'] <- 1/2.28
  ...
  for (scaling in c(0.5,1,2)){
    ...
    sumry.stats2 <- sumry.stats2 %>%
      mutate(scaling=scaling,vaccineImmunity='tau1=0\ntau2=b*theta2')
  ...
  for (scaling in c(0.5,1,2)){
    ...
    sumry.stats3 <- lapply(simdf.list3,sumry.cycl) %>% bind_rows()
    sumry.stats3 <- sumry.stats3 %>%
      mutate(scaling=scaling,vaccineImmunity='tau1=theta1\ntau2=0\nsigmaV=b*sigma2')
  ...
  save(simdf.meta.list,stats.sum,simdf.meta.list.vac.dur,
       stats.sum.vac.dur,file=paste0('sim_NewParms',idx,'.RData'))
}
