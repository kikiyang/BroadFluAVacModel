...
sirmod <- function(t,y,parms){
  with(as.list(c(y, parms)), {
    ...
    dV <- rho*S +sigma1*RV1+sigma2*RV2 -(mu+sigmaV)*V-
      beta2*(1-tau)*V*(I2+I12+IV2)-beta1*(1-tau2)*V*(I1+I21+IV1) #vaccinated
    
    dIV1 <- beta1*(1-tau2)*V*(I1+I21+IV1) - (gamma1/(1-eta2)+mu)*IV1
    dRVT1 <- gamma1/(1-eta2)*IV1 + rho2*RT1 - (sigmaV2+sigmaT+mu)*RVT1
    dRV1 <- sigmaT*RVT1 + rho2*R1-(sigma1+sigmaV2+mu)*RV1
    # dIV12 <- beta2*(1-theta1)*(1-tau)*RV1*(I2+I12+IV2) -
    ...
    dRV2 <- sigmaT*RVT2 +rho1*R2 -(sigma2+sigmaV1+mu)*RV2 #vaccinated individuals recovered from strain2 infection 
    
    # dIV21 <- beta1*(1-theta2)*(1-tau2)*RV2*(I1+I21+IV1) -(gamma1/(1-eta2)/(1-nu2)+mu)*IV21
    ...
    return(list(res,inci1=beta1*S*(I1+I21+IV1)+beta1*(1-theta2)*R2*(I1+I21+IV1)+
                  beta1*(1-tau2)*V*(I1+I21+IV1)+beta1*(1-theta2)*(1-tau2)*RV2*(I1+I21+IV1),
                inci2=beta2*S*(I2+I12+IV2)+beta2*(1-theta1)*R1*(I2+I12+IV2)+
                  beta2*(1-tau)*V*(I2+I12+IV2)+beta2*(1-theta1)*(1-tau)*RV1*(I2+I12+IV2)))
  })
}
sim <- function(rel.vac.rho,rel.parm,odefunc,start.cond,times.vec,parms.init){

  # strength/duration against only non-target clade is scaled
...
for (idx in 1:nrow(bestfitsparms)){
  ...
  save(simdf.meta.list,stats.sum,simdf.meta.list.vac.dur,
       stats.sum.vac.dur,file=paste0('sim_Scale1clade_',idx,'.RData'))
}
